    ...
    var request = method.request(options, function(response){
      ...
      if (code == 301){
        ...
        if (newURL && newURL !== ''){
          ...
          this.error('Got a 301 redirect, but couldn\'t find a URL to redirect to');
        }
      }
      ...
      response.on('end', function(){
        ...
        if (result === ''){
            ...
            setTimeout(function(){
                this.fetch(entry, pos);
            }.bind(this), (entry.wait || this.defaults.wait) * 1000);
...
Sieve.prototype.fill = function(entry){
  ...
  if (entry.url && entry.data){ 
    var results = this.template(entry.url, entry.data);
